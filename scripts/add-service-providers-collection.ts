/**
 * Creates the serviceProviders collection in PocketBase via REST API.
 * Usage: npx tsx scripts/add-service-providers-collection.ts
 */
import PocketBase from 'pocketbase';

const PB_URL = process.env.POCKETBASE_URL || 'http://127.0.0.1:8090';
const pb = new PocketBase(PB_URL);

async function main() {
  const authData = await pb.collection('_superusers').authWithPassword(
    process.env.POCKETBASE_ADMIN_EMAIL!,
    process.env.POCKETBASE_ADMIN_PASSWORD!
  );
  console.log('Authenticated as admin.');

  const token = pb.authStore.token;

  // Use raw fetch against PocketBase API to avoid SDK version schema issues
  const res = await fetch(`${PB_URL}/api/collections`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`,
    },
    body: JSON.stringify({
      name: 'serviceProviders',
      type: 'base',
      fields: [
        { name: 'name',              type: 'text',   required: true  },
        { name: 'serviceType',       type: 'text',   required: true  },
        { name: 'residentId',        type: 'text',   required: true  },
        { name: 'residentName',      type: 'text',   required: false },
        { name: 'apartment',         type: 'text',   required: false },
        { name: 'block',             type: 'text',   required: false },
        { name: 'accessDuration',    type: 'text',   required: false },
        { name: 'accessExpiry',      type: 'text',   required: false },
        { name: 'registrationToken', type: 'text',   required: true  },
        { name: 'tokenExpiry',       type: 'text',   required: false },
        { name: 'status',            type: 'text',   required: false },
        { name: 'photo',             type: 'file',   required: false, maxSelect: 1, maxSize: 5242880 },
        { name: 'hikvisionSyncStatus', type: 'json', required: false },
      ],
    }),
  });

  const data = await res.json();
  if (!res.ok) {
    console.error('PocketBase error:', JSON.stringify(data, null, 2));
    process.exit(1);
  }
  console.log('Collection "serviceProviders" created successfully! ID:', data.id);
}

main().catch(err => {
  console.error('Unexpected error:', err.message);
  process.exit(1);
});
